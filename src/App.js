...
import { operators, numbers, specials } from "./data";
...
function App() {
  ...
  const buttonClick = (button, update = true) => {
    ...
    if (numbers.includes(button)) {
      ...
      if (button.value !== "=") {
        if (!standardOperators.includes(currDisplay[currDisplay.length - 1])) {
          let data = buttonClick(operators[operators.length - 1], false);
          currHistory = data[0];
          currDisplay = data[1];
        }

        if (
          lastInput !== button.value &&
          standardOperators.includes(lastInput)
        ) {
          let changedHistory = [...displayHistory];
          changedHistory[changedHistory.length - 1] = button.value;
          currHistory = changedHistory;
          currDisplay =
            Number(displayHistory[displayHistory.length - 2]) +
            ` ${button.value}`;
      ...
      <div className="App">
        ...
        <div className="mainFlex">
          <div className="column">
            <Specials buttonClick={buttonClick} />
            <Numbers buttonClick={buttonClick} />
          </div>
          <div className="column">
            <Operators buttonClick={buttonClick} />
          </div>
        </div>
